...
class FoldPreprocessor:
    ...
    def fill_missing(self, X: np.ndarray) -> np.ndarray:
        ...
        for i in range(X.shape[0]):
            ...
            df.fillna(0, inplace=True) # 兜底极端全空情况
...
def train_and_evaluate(X: np.ndarray, y: np.ndarray):
    ...
    """
    # ============ 完全复现性保障 ============
    # 必须首先固定全局随机状态，确保即使外部未设置种子也能单独运行时复现
    RANDOM_SEED = 42
    ...
    # StratifiedKFold 分层划分也在此处使用 RANDOM_SEED 保证固定
    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=RANDOM_SEED)

    print(f"数据总形状: X={X.shape}, y={y.shape}")
    print(f"标签分布: {np.bincount(y)}")
    print("开始 5 折交叉验证 (模型: MiniRocketMultivariate)...\n")
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
        ...
        # 3. 转换数据维度适应 sktime (要求: [samples, features, timesteps])
        X_train_sktime = np.transpose(X_train_scaled, (0, 2, 1))
        X_test_sktime = np.transpose(X_test_scaled, (0, 2, 1))

        # 4. 初始化并训练 MiniRocket 特征提取器
        # MiniRocket 需要序列长度大于等于 9，且利用固定卷积核提取全局特征
        # 小结：所有随机性已完全固定
        # - StratifiedKFold: shuffle=True + random_state=42 固定划分
        # - FoldPreprocessor: 无随机性 (mean/std 计算完全确定)
        # - MiniRocketMultivariate: random_state=42 固定卷积核初始化
        # - RidgeClassifierCV: 确定性线性模型，无随机性
        # - 数据隔离: 每折独立拟合，训练集统计量不泄露到测试集
        ...
        # 5. 使用 RidgeClassifierCV 训练分类器 (Rocket的标配线性评估头)
        classifier = RidgeClassifierCV(alphas=np.logspace(-3, 3, 10))
        classifier.fit(X_train_features, y_train)

        # 6. 预测与评估
        y_pred = classifier.predict(X_test_features)
        # RidgeClassifier 没有 predict_proba，使用 decision_function 计算置信度用于 AUC
    ...
    # ================= 4. 指标汇总与绘图 =================
    print("========================================")
    print("5 折交叉验证全部完成！总耗时: {:.2f}s".format(time.time() - start_total_time))
    ...
    plt.title('5折交叉验证累计混淆矩阵 (MiniRocket)')
    plt.xlabel('预测标签')
    plt.ylabel('真实标签')
    plt.tight_layout()
...
if __name__ == "__main__":
    # 为了保证代码立刻可运行，这里生成符合要求的 Dummy Data 模拟你加载的数据
    # 实际应用时，请替换为使用 np.load() 加载你用预处理脚本生成的真实数据
    print("生成并加载测试数据集 (格式: [样本数, 4096, 23])...")
    num_samples = 200 # 减少模拟样本量以便快速跑通
    # 注: train_and_evaluate() 内部会重新设置 np.random.seed(42)，所以这里的种子设置可选
    # 为了独立性演示，这里仍设置，但主要保证来自 train_and_evaluate 内部
    np.random.seed(42)
